...
let resources = [
  {
    ...
    filename: path.join(pageDirname, "site-com-blog-about-assets-styles.css"),
  },
  {
    format: "jpg",
    urlPath: "/photos/me.jpg",
    isBinary: true,
    filename: path.join(pageDirname, "site-com-photos-me.jpg"),
  },
  {
    format: "js",
    urlPath: "/assets/scripts.js",
    filename: path.join(pageDirname, "site-com-assets-scripts.js"),
  },
  {
    format: "html",
    urlPath: "/blog/about",
    filename: path.join(pageDirname, "site-com-blog-about.html"),
  },
];
...
beforeAll(async () => {
  const sourcePageContent = await readFile(
    buildFixturesPath("."),
    pageFilename
  );
  const promises = resources.map((info) =>
    readFile(buildFixturesPath("expected"), info.filename, info.isBinary).then(
      (data) => ({ ...info, data })
    )
  );

  expectedPageContent = await readFile(
    buildFixturesPath("expected"),
    pageFilename
  );
...
describe("negative cases", () => {
  ...
  test("load page: no response", async () => {
    await expect(
      fsp.access(path.join(tmpDirPath, pageFilename))
    ...
    await expect(pageLoader(invalidBaseUrl, tmpDirPath)).rejects.toThrow(
      expectedError
    );
  ...
  test.each([404, 500])("load page: status code %s", async (code) => {
    ...
    await expect(pageLoader(url, tmpDirPath)).rejects.toThrow(new RegExp(code));
  });

  test("load page: file system errors", async () => {
    const rootDirPath = "/sys";
    await expect(pageLoader(pageUrl.toString(), rootDirPath)).rejects.toThrow();

    const filepath = buildFixturesPath(pageFilename);
    await expect(pageLoader(pageUrl.toString(), filepath)).rejects.toThrow(
      /ENOTDIR/
    );

    const notExistsPath = buildFixturesPath("notExistsPath");
    await expect(pageLoader(pageUrl.toString(), notExistsPath)).rejects.toThrow(
      /ENOENT/
    );
  });
});
...
describe("positive cases", () => {
  ...
  test.each(formats)("check .%s-resource", async (format) => {
    const { filename, data, isBinary } = resources.find(
  ...
  test("download to current workdir", async () => {
    ...
    await expect(
      fsp.access(path.join(cwd, pageFilename))
    ).resolves.not.toThrow();

    const loadedAssets = await fsp
      .readdir(path.join(cwd, pageDirname))
      .then((filenames) =>
        filenames.map((filename) => path.join(pageDirname, filename)).sort()
      );
